...
import type { FormikFieldProps, TypeaheadOption } from '../types';
...
describe('TypeaheadField', () => {
  it('renders with label', () => {
    ...
    render(<TypeaheadField name="broker" label="Broker" options={mockOptions} formik={formik} />);
  ...
  it('does not write typed text into formik when free text is disabled', async () => {
    const user = userEvent.setup();
    const setFieldValue = jest.fn();
    const formik = createMockFormik({ setFieldValue });
  ...
  it('renders the selected option label when formik stores the option value', async () => {
    const formik = createMockFormik({
      values: { broker: 'xyz-transport' },
    });

    render(<TypeaheadField name="broker" label="Broker" options={mockOptions} formik={formik} />);

    await waitFor(() => {
      expect(screen.getByRole('combobox', { name: /broker/i })).toHaveValue('XYZ Transport');
    });
  });
  ...
  it('does not call onInputValueChange when an option is selected', async () => {
    const user = userEvent.setup();
  ...
  it('calls onInputValueChange when the user types', async () => {
    const user = userEvent.setup();
    const onInputValueChange = jest.fn();
    const formik = createMockFormik();
    ...
    expect(onInputValueChange).toHaveBeenCalledWith('ABC');
  });
});
