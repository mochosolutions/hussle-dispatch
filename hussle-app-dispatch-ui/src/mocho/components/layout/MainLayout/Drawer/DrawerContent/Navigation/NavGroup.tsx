...
const NavGroup = ({
  ...
  const getItemIcon = () => {
    const iconProp = currentItem?.icon;
  ...
  const renderNavCollapse = (menuItem: NavItemType) => {
    ...
    <Fragment key={i}>
      ...
      {itemRem?.elements?.map((menuEl) => {
        switch (menuEl.type) {
          ...
          default:
            return (
              <Typography key={menuEl.id} variant="h6" color="error" align="center">
                Menu Items Error
              </Typography>
            );
        }
    ...
    <>
      {menuOrientation === MenuOrientation.VERTICAL || downLG ? (
        ...
        <List>
          ...
          >
            {itemIcon && (
              <ListItemIcon sx={{ minWidth: 28, marginRight: 1 }}>
