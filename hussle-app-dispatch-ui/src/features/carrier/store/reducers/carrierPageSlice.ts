...
import type { CarrierOnboardingStatus, CreateCarrierInput, UpdateCarrierInput } from '../../types';
...
interface CarrierPageState {
  ...
  total: number;
  typeFilter: string;
  onboardingByCarrierId: Record<string, CarrierOnboardingStatus>;
}
...
interface FetchCarrierOnboardingRequestPayload {
  id: string;
}

interface FetchCarrierOnboardingSuccessPayload {
  id: string;
  onboarding: CarrierOnboardingStatus;
}
...
const carrierPageSlice = createSlice({
  ...
  reducers: {
    ...
    fetchCarrierOnboardingRequest(
      state,
      action: PayloadAction<FetchCarrierOnboardingRequestPayload>,
    ) {
      const key = `onboarding:${action.payload.id}`;
      setPending(state, { key });
    },
    fetchCarrierOnboardingSuccess(
      state,
      action: PayloadAction<FetchCarrierOnboardingSuccessPayload>,
    ) {
      const key = `onboarding:${action.payload.id}`;
      setFulfilled(state, { loadingKey: key, errorKey: key });
      state.onboardingByCarrierId[action.payload.id] = action.payload.onboarding;
    },
    fetchCarrierOnboardingFailure(state, action: PayloadAction<FailurePayload>) {
      const key = action.payload.id ? `onboarding:${action.payload.id}` : 'onboarding';
      setRejected(state, {
        loadingKey: key,
        errorKey: key,
        failureMessage: action.payload.error,
