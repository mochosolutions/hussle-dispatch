...
export const asyncStatusSlice = createSlice({
  ...
  reducers: {
     ...
     */
    setAsyncStatus(state, action: PayloadAction<{ key: string; status: LoadingState }>) {
     ...
     */
    setAsyncError(state, action: PayloadAction<{ key: string; message: string }>) {
...
export const { setAsyncStatus, setAsyncError, clearAsyncError } = asyncStatusSlice.actions;
 ...
 */
export function selectLoading(statusState: AsyncStatusState, key: string): LoadingState {
 ...
 */
export function selectError(statusState: AsyncStatusState, key: string): string {
...
export function makeSelectError(key: string) {
  return (statusState: AsyncStatusState): string => statusState.errors[key] ?? '';
}
 ...
 */
export function* callApiWithStatus<T>(opts: CallApiWithStatusOpts<T>): Generator<SagaEffect, T, T> {
