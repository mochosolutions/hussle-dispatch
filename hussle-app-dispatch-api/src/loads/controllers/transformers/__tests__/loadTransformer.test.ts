...
const buildLoad = (overrides?: Partial<LoadWithRelations>): LoadWithRelations => {
  const base: LoadWithRelations = {
    ...
    customerRate: null,
    carrierRate: null,
    // Persisted output cache columns — present on the Prisma row but no
    // longer read by the transformer (US-11). Kept here so the fixture is
    // type-compatible with the Load model until US-15 drops them.
    dispatchFee: null,
    partnerSplit: null,
    ratePerMile: null,
